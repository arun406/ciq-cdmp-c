    ...
    private static List<LineDTO> prepareLines(List<String[]> records) {
        return records.stream()
                ...
                .map(record -> {
                    ...
                    dto.setFOW(record[16]);
                    dto.setDEP(record[17]);
                    dto.setARR(record[18]);
                    dto.setAWR(record[19]);
                    dto.setFIW(record[20]);
                    dto.setRCF(record[21]);
                    dto.setNFD(record[22]);
                    dto.setAWD(record[23]);
                    dto.setDLV(record[24]);
                    dto.setRCFT(record[25]);
                    dto.setTFDT(record[26]);
                    dto.setRCTT(record[27]);
                    dto.setARRT(record[28]);
                    dto.setDEPT(record[29]);
                    dto.setWtInd(record[30]);
                    dto.setVolInd(record[31]);
                    dto.setEFreightInd(record[32]);
                    dto.setNotes(record[33]);
                    return dto;
                })
                .collect(Collectors.toList());
    }
