...
import com.aktimetrix.service.ciq.cdmpc.messaging.MilestoneChangeStatus;
import com.aktimetrix.service.ciq.cdmpc.messaging.RMPMessageDTO;
import com.aktimetrix.service.ciq.cdmpc.messaging.RMPMilestoneDTO;
...
@Component
@Slf4j
public class CiQRMP12MessageGenerator implements MessageGenerator {

    public static final String CIQ_NOTIFICATION_RMP_TEMPLATE = "rmp.ftl";
    private String messageType = "RMP";
    ...
    private File generatePayload(Object object) {
        if (Arrays.asList("RMP").contains(messageType)) {
            // Route map level messages
            ProcessPlanDTO plan = (ProcessPlanDTO) object;
            RMPMessageDTO rmpMessageDTO = prepareRMPMessageDTO(plan);
            int messageCounter = getMessageCounter(rmpMessageDTO.getAirWaybillPrefix(), rmpMessageDTO.getAirWaybillNumber(), messageType);
            log.debug("message sequence {}", messageCounter);
            rmpMessageDTO.setMessageCounter(messageCounter + "");
            FileWriter fileWriter = null;
            try {
                Template template = freeMarkerConfigurer.getConfiguration().getTemplate(CIQ_NOTIFICATION_RMP_TEMPLATE);
                // write output into a file:
                String ciqMessageFileName = new CiQFileNameGenerator(ciQProperties).generateFileName(rmpMessageDTO);
                log.debug("ciq message filename: {}", ciqMessageFileName);

                File file = new File(ciqMessageFileName);
                fileWriter = new FileWriter(file);
                Map<String, Object> data = new HashMap<String, Object>();
                data.put("rmpMessageDTO", rmpMessageDTO);
...
//        rmpMessageDTO.setTotalVolume(Double.valueOf((String) metadata.get("reservationVolume")));
        ...
        if (!plan.getUpdateFlag().equals("U")) {
